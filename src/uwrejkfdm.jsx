...
import { 
  ...
  FiCalendar, FiFileText, FiFolder, FiType, FiAlertCircle, FiSliders, 
  FiLayers, FiDroplet, FiFeather, FiCreditCard, FiGrid, FiBox, 
...
useEffect(() => {
    ...
    if (!userData) {
      navigate('/signin'); // Redirect to login if no user data
    } else {
      setUser(userData);
    }
  }, [navigate]);
    ...
    <div className={`dashboard ${darkMode ? 'dark' : ''} ${sidebarOpen ? 'sidebar-open' : ''}`}>
      ...
      <div className={`sidebar ${sidebarOpen ? '' : 'sidebar-closed'} ${mobileSidebarOpen ? 'mobile-open' : ''}`}>
        ...
        <nav className="sidebar-content">
          ...
          <div className="sidebar-section">
            <h2 className="sidebar-section-title">UI ELEMENTS</h2>
            <ul className="sidebar-menu">
              <NavItem icon={<FiType />} text="Typography" />
              <NavItem icon={<FiAlertCircle />} text="Alerts" />
              <NavItem icon={<FiSliders />} text="Buttons" />
              <NavItem icon={<FiLayers />} text="Content" />
              <NavItem icon={<FiDroplet />} text="Colors" />
              <NavItem icon={<FiFeather />} text="Icons" />
              <NavItem icon={<FiCreditCard />} text="Card" />
              <NavItem icon={<FiGrid />} text="Widgets" />
              <NavItem icon={<FiBox />} text="Components" />
            </ul>
          </div>
